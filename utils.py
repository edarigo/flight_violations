...
def str_to_time(timestamp,tzsource=None):
    ...
    if time.tzinfo != None:
        ...
        return timezone(tzsource).localize(time)
    elif type(tzsource) == datetime.datetime:
        return time.replace(tzinfo=tzsource.tzinfo)
    else:
        return time
...
def daytime(time,daycycle):
    ...
    month_day = str(time.strftime('%m-%d'))

    timezone = daycycle['timezone']

    sunset = daycycle[year][month_day]['sunset'] if year in daycycle else None

    sunset_time = str_to_time(year+' '+month_day+' '+sunset,timezone)

    sunrise = daycycle[year][month_day]['sunrise'] if year in daycycle else None

    sunrise_time = str_to_time(year+' '+month_day+' '+sunrise,timezone)

    if time.tzinfo == None:
        time_w_zone = str_to_time(str(time),timezone)
    else:
        time_w_zone = time

    if time_w_zone < sunrise_time:
        return False
    elif time_w_zone > sunset_time:
        return False
    elif time_w_zone > sunrise_time and time_w_zone < sunset_time:
        return True
    else:
        return False
